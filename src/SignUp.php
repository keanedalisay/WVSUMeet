...
class SignUp
{
  ...
  public function __construct() {
    $this->input_name = filter_input(INPUT_POST, 'user-name', FILTER_SANITIZE_FULL_SPECIAL_CHARS);
    $this->input_wvsuid = filter_input(INPUT_POST, 'user-wvsuid', FILTER_SANITIZE_FULL_SPECIAL_CHARS);

    $this->input_password = filter_input(INPUT_POST, 'user-password', FILTER_SANITIZE_FULL_SPECIAL_CHARS);

    $this->postUserData();

    $_SESSION['user-name'] = $this->input_name;
    $_SESSION['user-wvsuid'] = $this->input_wvsuid;
    $_SESSION['has-logged-in'] = true;
